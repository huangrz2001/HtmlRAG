...
async def rewrite_query_ollama_async(dialogue, final_query, model="qwen2.5:32b", max_new_tokens=1024):
    fallback_rewrite = final_query
    banned_phrases = [
        "你好", "您好", "hi", "hello", "哈喽", "在吗", "喂", "请问在吗", "有人吗", "hello？",
        "你是谁", "你是人吗", "你是机器人吗", "你是AI吗", "你叫什么", "你是客服吗", "你是智能助手吗", "你是人工的吗", "你能听懂我说话吗",
        ...
        async with sem:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(OLLAMA_URL, json=payload, timeout=60) as resp:
                    ...
                    result = await resp.json()
                    rewritten = result["choices"][0]["message"]["content"].strip()
                    rewritten = rewritten.split("</think>")[-1].strip() if "</think>" in rewritten else rewritten
                    logger.debug(f"{final_query} 重写成功，用时 {time.time() - start:.2f}s，结果：{rewritten}")
                    return rewritten or fallback_rewrite
    except Exception as e:
